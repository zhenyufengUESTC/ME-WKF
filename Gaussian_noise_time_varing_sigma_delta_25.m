...
%         delta_t = -delta + 2 * delta * rand(1);% [-1,1]区间的均匀分布随机数*delta_t
        %                 delta_t = 0;
%         F = [0.9802,  0.0196; 0, 0.9802];%.1196  .98022 +0.099*25*delta_t
...
% save Table_2_time_varing_sigma_delta_25.mat
%% 绘图

figure; hold on;%
plot(10*log10(mean(Err_MSE_KF)));plot(10*log10(mean(Err_MEE_KF)),'r-+');plot(10*log10(mean(Err_WKF)),'g-+');%plot(10*log10(mean(Err_ME_WKF)));plot(10*log10(mean(Err_ME_WKF1)));
plot(10*log10(mean(Err_ME_WKF2)));
...
legend('Err-MSE-KF','Err-MEE-KF','Err-WKF','Err_ME_WKF2','Err_ME_WKF4','Err_ME_WKF6','Err_ME_WKF8','Err_ME_WKF10');%,'Err_ME_WKF_0.5',mean(mean(Err_ME_WKF)),'Err_ME_WKF1'mean(mean(Err_ME_WKF1)),


% [mean(mean(Err_MSE_KF)),mean(mean(Err_MEE_KF)),mean(mean(Err_WKF)),mean(mean(Err_ME_WKF2)),mean(mean(Err_ME_WKF4)),mean(mean(Err_ME_WKF6)),mean(mean(Err_ME_WKF8)),mean(mean(Err_ME_WKF10))]
