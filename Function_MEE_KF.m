function [xe_MEE,Pk_MEE] = Function_MEE_KF(n,m,F,xe_MEE,Pk_MEE,H,yy_MEE,R,Q,sigma_MEE)
%% 状态噪声 观测噪音 非高斯 +  时间对比
%% 设定初始状态：
%% MEE_KF循环计算
epsilon = 1e-6;
...
Pke = F * Pk_MEE * F' + Q;
xeii_t0 = F * xe_MEE; xeii_t = xeii_t0 ;
theta_p =(chol(Pke))' ;     theta_r =(chol(R))' ;
theta = [theta_p zeros(n, m); zeros(m, n)  theta_r];
D  = inv(theta) * [xeii_t0 ; yy_MEE];  W  = inv(theta) * [eye(n); H];
for b = 1 : 20
    xeii_t1 = xeii_t;
    %error
    e = D - W * xeii_t1;
...
%     Psi_k = zeros(n+m,n+m);
    ...
    xeii_t = xeii_t0 + Ke * (yy_MEE - H * xeii_t0);
    %% loop counter   
